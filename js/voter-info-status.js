(function () {
  ...
  function metric(label, value, filter) {
    return `<button type="button" class="voter-info-metric" data-filter="${esc(filter)}" data-info-filter="${esc(filter)}" aria-label="Show ${esc(label)} voters"><strong>${esc(formatNumber(value))}</strong><span>${esc(label)}</span></button>`;
  }
