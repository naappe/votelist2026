(function(){
  ...
  function val(v){return clean(v).toLowerCase()}
  function assigned(row){const x=val(row.vote_assigned_by);return x&&x!=='naappe@gmail.com'}
  function will(row){return val(row.vote_status)==='will-vote'}
  function noVote(row){return ['no-vote','not-vote','no vote','not vote'].includes(val(row.vote_status))}
  function reached(row){return val(row.reach_status)==='reached'}
  ...
  function addStyles(){if(document.getElementById('assignedResultSimpleStyles'))return;const s=document.createElement('style');s.id='assignedResultSimpleStyles';s.textContent='.assigned-result-summary{display:none!important}.assignment-status-chip{display:inline-flex;width:max-content;margin:6px 0 8px;padding:6px 9px;border-radius:999px;font-size:11px;font-weight:950}.assignment-status-chip.will{background:#dcfce7;color:#166534}.assignment-status-chip.no{background:#fee2e2;color:#991b1b}.assignment-status-chip.pending{background:#fffbeb;color:#92400e}.assignment-status-chip.notreached{background:#fef2f2;color:#991b1b}.assignment-hidden{display:none!important}#assignedTaskFilters{display:none!important}';document.head.appendChild(s)}
  function status(row){if(will(row))return'will';if(noVote(row))return'no';if(reached(row))return'pending';return'notreached'}
  function chip(row){const st=status(row);if(st==='will')return'<span class="assignment-status-chip will">Will Vote</span>';if(st==='no')return'<span class="assignment-status-chip no">Not Vote</span>';if(st==='pending')return'<span class="assignment-status-chip pending">Reached · Result Pending</span>';return'<span class="assignment-status-chip notreached">Need Call / Visit</span>'}
  function markCards(rows){const map=new Map(rows.map(r=>[String(r.id),r]));document.querySelectorAll('.admin-assigned-card[data-open-voter]').forEach(card=>{const row=map.get(String(card.getAttribute('data-open-voter')));if(!row)return;card.dataset.assignmentStatus=status(row);card.querySelectorAll('.assignment-status-chip').forEach(x=>x.remove());const box=card.querySelector('.assigned-result-box');if(box)box.insertAdjacentHTML('afterend',chip(row))})}
  ...
  function render(rows){const title=document.getElementById('sectionTitle');const total=document.getElementById('sectionTotal');const msg=document.getElementById('statusMessage');if(!title||title.textContent!=='Assigned Results')return;removeDuplicatePanels();const a=rows.length,w=rows.filter(will).length,no=rows.filter(noVote).length,pendingReached=rows.filter(r=>!will(r)&&!noVote(r)&&reached(r)).length,needCall=rows.filter(r=>!will(r)&&!noVote(r)&&!reached(r)).length,open=pendingReached+needCall;if(total)total.textContent=`${n(a)} assigned · ${n(w)} will vote · ${n(no)} not vote · ${n(open)} open`;if(msg){msg.textContent=`Assigned Result: ${n(w)} Will Vote, ${n(no)} Not Vote, ${n(pendingReached)} Reached Pending, ${n(needCall)} Need Call/Visit.`;msg.className='status-message ok'}markCards(rows)}
