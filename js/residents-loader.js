document.addEventListener('DOMContentLoaded', async function () {
  ...
  function buildNav(){
    ...
    nav.innerHTML=[['voters','Residents'],['assign','Assign'],['calls','Calls'],['votes','Votes'],['visits','Visits'],['transport','Transport'],['insights','Insights']].map(x=>'<a class="btn '+(x[0]===section?'active':'')+'" href="residents.html?party='+encodeURIComponent(party)+'&section='+x[0]+'&v=statsfix1">'+x[1]+'</a>').join('')+'<a class="btn" href="index.html?v=statsfix1">Logout</a>';
  }
  ...
  async function loadRows(){
    ...
    while(true){
      ...
      const res=await q;
      if(res.error){setStatus('Supabase error: '+res.error.message,true);return;}
      out=out.concat(res.data||[]);
      if(!res.data||res.data.length<size) break;
      from+=size;
    }
    allRows=out;
    rows=allRows.filter(sectionOk);
    buildHouseOptions();
    updateStats(allRows);
    applyFilter('all');
  ...
  function updateStats(data){
    ...
    setNum('total',base.length);
    setNum('will',base.filter(r=>r.vote_status==='will-vote').length);
    setNum('notvote',base.filter(r=>['no-vote','not-vote'].includes(r.vote_status)).length);
    setNum('pending',base.filter(r=>r.vote_status==='pending').length);
    setNum('need',base.filter(r=>r.reach_status!=='reached'||r.phone_status==='need-call'||r.d2d_status==='not-visited').length);
  }
  ...
  function applyFilter(filter){
    ...
    if(currentFilter==='pending') data=data.filter(r=>r.vote_status==='pending');
  ...
  function installEditor(){
    ...
    modal.innerHTML='<article class="edit-card"><div class="edit-head"><div><h2 id="editName">Update resident</h2><p id="editMeta"></p></div><button id="editClose" class="btn" type="button">Close</button></div><form id="editForm" class="edit-form"><label data-field="vote">Vote Status<select id="editVote"><option value="pending">Pending</option><option value="not-decided">Not Decided</option><option value="will-vote">Will Vote</option><option value="no-vote">Not Vote</option></select></label><label data-field="phone">Phone Status<select id="editPhone"><option value="need-call">Need Call</option><option value="called">Called</option><option value="busy">Busy</option><option value="switched-off">Switched Off</option><option value="disconnected">Disconnected</option><option value="wrong-number">Wrong Number</option><option value="out-of-range">Out Of Range</option><option value="no-phone">No Phone</option></select></label><label data-field="reach">Reach Status<select id="editReach"><option value="not-reached">Not Reached</option><option value="reached">Reached</option></select></label><label data-field="d2d">D2D Status<select id="editD2D"><option value="not-visited">Not Visited</option><option value="visited">Visited</option><option value="not-home">Not Home</option><option value="follow-up">Follow Up</option></select></label><label data-field="transport">Transport Status<select id="editTransport"><option value="not-needed">Not Needed</option><option value="need-transport">Need Transport</option><option value="arranged">Arranged</option><option value="picked-up">Picked Up</option></select></label><label data-field="support">Support Level<select id="editSupport"><option value="normal">Normal</option><option value="guaranteed">Guaranteed</option></select></label><label data-field="assigned">Assigned By<input id="editAssigned" type="text" placeholder="Name or comma separated names"></label><label data-field="remarks">Remarks<textarea id="editRemarks" placeholder="Write remarks"></textarea></label><div class="edit-actions"><button class="btn active" type="submit">Save Update</button><button id="editCancel" class="btn" type="button">Cancel</button></div></form></article>';
  ...
  function openEditor(id){current=allRows.find(r=>String(r.id)===String(id)); if(!current)return; document.getElementById('editName').textContent=current.name||'Update resident'; document.getElementById('editMeta').textContent=(current.house||'-')+' · '+(current.phone||'No phone'); document.getElementById('editVote').value=current.vote_status||'pending'; document.getElementById('editPhone').value=current.phone_status||'need-call'; document.getElementById('editReach').value=current.reach_status||'not-reached'; document.getElementById('editD2D').value=current.d2d_status||'not-visited'; document.getElementById('editTransport').value=current.transport_status||'not-needed'; document.getElementById('editSupport').value=current.support_level||'normal'; document.getElementById('editAssigned').value=current.vote_assigned_by||''; document.getElementById('editRemarks').value=current.remarks||''; applySectionFields(); document.getElementById('editModal').classList.add('open');}

  function applySectionFields(){
    const sectionFields={assign:['assigned','remarks'],calls:['phone','reach','remarks'],votes:['vote','support','transport','remarks'],visits:['d2d','remarks'],transport:['transport','remarks'],insights:['remarks'],voters:['vote','phone','reach','d2d','transport','support','assigned','remarks']};
    const keep=sectionFields[section]||sectionFields.voters;
    document.querySelectorAll('#editForm [data-field]').forEach(label=>{const show=keep.includes(label.dataset.field); label.style.display=show?'':'none'; label.querySelectorAll('input,select,textarea').forEach(el=>{el.disabled=!show;});});
  }

  function closeEditor(){document.getElementById('editModal').classList.remove('open');current=null; document.querySelectorAll('#editForm [data-field] input,#editForm [data-field] select,#editForm [data-field] textarea').forEach(el=>el.disabled=false);}

  async function saveEditor(e){e.preventDefault(); if(!current)return; const assigned=document.getElementById('editAssigned').value.trim(); const patch={remarks:document.getElementById('editRemarks').value}; if(section==='assign'||section==='voters'){patch.vote_assigned_by=assigned||null; patch.vote_assigned_at=assigned?new Date().toISOString():null;} if(section==='calls'||section==='voters'){patch.phone_status=document.getElementById('editPhone').value; patch.reach_status=document.getElementById('editReach').value;} if(section==='votes'||section==='voters'){patch.vote_status=document.getElementById('editVote').value; patch.support_level=document.getElementById('editSupport').value; patch.transport_status=document.getElementById('editTransport').value;} if(section==='visits'||section==='voters'){patch.d2d_status=document.getElementById('editD2D').value;} if(section==='transport'){patch.transport_status=document.getElementById('editTransport').value;} if(patch.vote_status==='will-vote'||patch.vote_status==='no-vote'||patch.support_level==='guaranteed'||patch.phone_status==='called')patch.reach_status='reached'; setStatus('Saving update...'); const res=await client.from('campaign').update(patch).eq('id',current.id).select().single(); if(res.error){setStatus('Save failed: '+res.error.message,true);return;} allRows=allRows.map(r=>String(r.id)===String(current.id)?Object.assign({},r,res.data||patch):r); rows=allRows.filter(sectionOk); buildHouseOptions(); updateStats(allRows); applyFilter(currentFilter); closeEditor(); setStatus('Saved update.');}
